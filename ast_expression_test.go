...
func TestEnclosure(t *testing.T) {
	doTests(t, []sourceFn{
		...
		{`(yield nonlocal)`, func(t *test, tk Tokens) { // 29
			t.Err = Error{
				Err: Error{
					Err: Error{
						Err: Error{
		...
		{`(nonlocal for)`, func(t *test, tk Tokens) { // 30
			t.Err = Error{
				Err: Error{
					Err: Error{
						Err: wrapConditionalExpressionError(Error{
		...
		{`[a for b in c d]`, func(t *test, tk Tokens) { // 34
			t.Err = Error{
		...
		{`{a for b in c d}`, func(t *test, tk Tokens) { // 38
			t.Err = Error{
