...
func TestEnclosure(t *testing.T) {
	doTests(t, []sourceFn{
		...
		{`(yield nonlocal)`, func(t *test, tk Tokens) { // 34
			t.Err = Error{
				Err: Error{
					Err: Error{
						Err: Error{
		...
		{`(nonlocal for)`, func(t *test, tk Tokens) { // 35
			t.Err = Error{
				Err: Error{
					Err: Error{
						Err: wrapConditionalExpressionError(Error{
		...
		{`(nonlocal)`, func(t *test, tk Tokens) { // 36
			t.Err = Error{
				Err: Error{
					Err: Error{
						Err: wrapConditionalExpressionError(Error{
		...
		{`[a for b in c d]`, func(t *test, tk Tokens) { // 39
			t.Err = Error{
		...
		{`{a:nonlocal}`, func(t *test, tk Tokens) { // 41
			t.Err = Error{
				Err: Error{
					Err: Error{
						Err: Error{
		...
		{`{a for b in c d}`, func(t *test, tk Tokens) { // 43
			t.Err = Error{
